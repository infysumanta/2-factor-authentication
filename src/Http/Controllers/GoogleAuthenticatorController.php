...
class GoogleAuthenticatorController extends Controller
{
    ...
    {
        $google = new PHPGangsta_GoogleAuthenticator();
//        if ($google->verifyCode(config('admin.extensions.laravel-admin-google-authenticator.secret', ''), $code, 1)) {
//            return true;
//        }

        $row  = Administrator::where('username',request('username'))->first();

        if ($google->verifyCode($row->google_auth, $code, 1)) {
            return true;
        }
